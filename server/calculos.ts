// Serviço para cálculos de precificação no servidor
...
export function calcularPrecoProduto(params: CalculoProdutoParams): ResultadoCalculo {
  ...
  const custosAdicionais = custos.reduce((acc, custo) => acc + custo, 0);
  ...
  const custoBase = valorCusto + frete + custosAdicionais;
  ...
  return {
    ...
    valorParcela,
    custoTotal: custoBase, // Usando custoBase que inclui valorCusto + frete + outros custos
    taxaTotal
  };
}
...
export function calcularPrecoServico(params: CalculoServicoParams): ResultadoCalculo {
  ...
  const custosAdicionais = custos.reduce((acc, custo) => acc + custo, 0) + custoDeslocamento;
  ...
  const custoBase = valorCusto + custosAdicionais;
  ...
  return {
    ...
    lucroPercentual: lucroPercentualEfetivo,
    valorParcela,
    custoTotal: custoBase, // Usando custoBase que inclui valorCusto + custos adicionais
    taxaTotal
  };
}
...
export function calcularPrecoAluguel(params: CalculoAluguelParams): ResultadoCalculo {
  ...
  // Base para cálculo mensal (inclui valor mensal de recuperação + custos distribuídos pelos meses)
  const custoBase = valorMensalRecuperacao + (custosAdicionais / tempoContratoMeses);
  ...
  return {
    ...
    lucroPercentual: lucroPercentualEfetivo,
    valorParcela,
    custoTotal: custoBase, // Usando custoBase como custo total mensal
    taxaTotal
  };
}
...
export function calcularPrecoMarketplace(params: CalculoMarketplaceParams): ResultadoCalculo {
  ...
  return {
    ...
    lucroPercentual: lucroPercentualEfetivo,
    valorParcela,
    custoTotal: custoBase, // Usando custoBase que inclui valorCusto + frete + custos adicionais
    taxaTotal
  };
}