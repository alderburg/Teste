...
import { Building, Eye, EyeOff, Loader2, ArrowLeft, ChevronLeft, ChevronRight, Check, ShieldCheck, BarChart2, Users, LineChart, Briefcase, ArrowUp, TrendingUp, Clock, X } from "lucide-react";
...
export default function SignupPage() {
  ...
  useEffect(() => {
    ...
    // Verificar parâmetro de teste para mostrar a tela de verificação
    const params = new URLSearchParams(window.location.search);
    if (params.get('test_verification') === 'true') {
      setEmailVerificationSent(true);
      setRegisteredEmail('teste@email.com');
      setVerificado(true);
      return;
    }
    
    // Se for um teste com skip_auth, não faz o redirecionamento mesmo que o usuário esteja logado
    if (params.get('skip_auth') === 'true') {
      console.log("Ignorando verificação de autenticação para teste");
      setVerificado(true);
      return;
    }
  ...
  async function onSubmit(data: SignupFormValues) {
    ...
    try {
      // Formatar o nome do usuário antes de enviar
      const formattedName = formatName(data.username);
      ...
      const response = await fetch('/api/register', {
        ...
        body: JSON.stringify({
          username: formattedName, // Usando o nome formatado
          email: data.email,
      ...
      // Armazenar o email registrado para exibir na mensagem de confirmação
      setRegisteredEmail(data.email);
      
      // Mostrar a tela de confirmação de email
      setEmailVerificationSent(true);
      ...
      // Executar logout no servidor para invalidar a sessão
      try {
      ...
      toast({
        title: "Conta criada",
        description: "Enviamos um email de confirmação para você. Por favor, verifique seu email antes de fazer login.",
      });
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Ocorreu um erro durante o cadastro. Por favor, tente novamente.",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  // Se for mobile, renderizar o componente MobileSignup
  if (isMobile) {
    // Importar dinamicamente sem mostrar indicador de carregamento
    const MobileSignup = React.lazy(() => import('./MobileSignup'));
  ...
  return (
    <div className="h-screen flex flex-col md:flex-row bg-white overflow-auto">
      {/* Seção de apresentação - à esquerda */}
      <div className="hidden md:flex md:w-1/2 lg:w-3/5 xl:w-2/3 2xl:w-3/4 relative overflow-auto">
      ...
      <div className="w-full md:w-1/2 lg:w-2/5 xl:w-1/3 2xl:w-1/4 p-3 md:p-5 lg:p-6 flex flex-col justify-center bg-white overflow-auto min-h-screen">
        <div className="mx-auto w-full max-w-md py-2">
          ...
          <div className="h-[24px] [@media(max-width:1370px)]:h-[50px]"></div>

          {/* Título e subtítulo - condicional baseado no estado de verificação */}
          <div className="mb-3 [@media(max-width:1370px)]:pt-6">
            {emailVerificationSent ? (
              <>
                <h2 className="text-xl font-bold mb-1 text-gray-900">Verifique seu email</h2>
                <div className="text-gray-600 text-xs mb-6">
                  Falta pouco! Complete a verificação para acessar sua conta.
                </div>
              </>
            ) : (
              <>
          ...
          {/* Formulário de cadastro ou mensagem de confirmação */}
          {emailVerificationSent ? (
            <div className="w-full max-w-md bg-white rounded-xl p-6 border border-gray-100 animate-fadeSlideUp">
              ...
              <div className="pt-4 space-y-4">
                ...
                <Button 
                  variant="outline"
                  className="w-full border-indigo-300 text-indigo-700 hover:bg-indigo-50 h-11 rounded-xl" 
                  onClick={async () => {
                    try {
                      setIsLoading(true);
                  ...
                  <FormItem className="flex flex-row items-start space-x-2 space-y-0 mt-2">
                    ...
                    <div className="space-y-1 leading-none">
                      <FormLabel className="text-sm font-normal">
                        Eu concordo com os <span 
                          onClick={(e) => {
                            e.preventDefault();
                            const button = document.querySelector('button[data-terms-button="true"]');
                         ...
                         >
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            const button = document.querySelector('button[data-privacy-button="true"]');
                            if (button instanceof HTMLElement) button.click();
                          }}
                          className="text-primary font-medium cursor-pointer"
                        >
          ...
          {/* Link para login - mostrado apenas se não estiver na tela de confirmação */}
          {!emailVerificationSent && (
