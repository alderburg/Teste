...
import { DollarSign, Download, Upload, PlusCircle, Search, Calculator, Edit, Settings, ChevronLeft, ChevronRight, Briefcase, Clock, Wrench } from "lucide-react";
...
export default function PrecificacaoServicosPage() {
  ...
  const [unitarioRating, setUnitarioRating] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('unitarioServicosRating');
      const version = localStorage.getItem('unitarioServicosVersion');
  ...
  const [importacaoRating, setImportacaoRating] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('importacaoServicosRating');
      const version = localStorage.getItem('importacaoServicosVersion');
  ...
  const [moduleUpdated, setModuleUpdated] = useState(() => {
    const unitarioOldVersion = localStorage.getItem('unitarioServicosVersion');
    const importacaoOldVersion = localStorage.getItem('importacaoServicosVersion');
  ...
  const handleUnitarioRating = (rating: number) => {
    ...
    localStorage.setItem('unitarioServicosRating', rating.toString());
    localStorage.setItem('unitarioServicosVersion', UNITARIO_VERSION);
    
    toast({
      title: "Avaliação salva",
      description: `Sua avaliação de ${rating} ${rating === 1 ? 'estrela' : 'estrelas'} para o Módulo de Precificação Unitária de Serviços foi registrada. Obrigado!`,
    });
  ...
  const handleImportacaoRating = (rating: number) => {
    ...
    // Salvar no localStorage
    localStorage.setItem('importacaoServicosRating', rating.toString());
    localStorage.setItem('importacaoServicosVersion', IMPORTACAO_VERSION);
    
    toast({
      title: "Avaliação salva",
      description: `Sua avaliação de ${rating} ${rating === 1 ? 'estrela' : 'estrelas'} para o Módulo de Importação de Serviços foi registrada. Obrigado!`,
    });
    // Aqui você poderia enviar a avaliação para o backend
  };
  ...
  // Serviços de exemplo - vazio para demonstração
  const servicosOriginais: Servico[] = [];
  
  // Estado para serviços filtrados
  const [servicosFiltrados, setServicosFiltrados] = useState<Servico[]>(servicosOriginais);
  ...
  const handleBusca = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Filtrar serviços
    if (valor.trim() === "") {
      setServicosFiltrados(servicosOriginais);
    } else {
      const resultados = servicosOriginais.filter(servico => 
        servico.nome.toLowerCase().includes(valor.toLowerCase()) || 
        servico.id.toLowerCase().includes(valor.toLowerCase()) ||
        servico.categoria.toLowerCase().includes(valor.toLowerCase())
      );
  ...
  useEffect(() => {
    ...
    const isTabelaVazia = servicosFiltrados.length === 0;
    ...
    <div className="space-y-6">
      <div className="flex items-start sm:items-center justify-between pt-3 sm:pt-0">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight mt-1 sm:mt-0">
            <span className="hidden sm:inline">Precificação de Serviços</span>
            <span className="inline sm:hidden">Precificação - Serviços</span>
          </h2>
          <p className="text-gray-500">Gerencie os preços dos seus serviços</p>
        </div>
        <div className="flex space-x-2 mt-2 sm:mt-0">
          <Link href="/cadastros/servicos">
            <Button className="flex items-center bg-teal-600 hover:bg-teal-700 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="flex flex-col">
          <CardHeader>
            ...
            <CardDescription>
              Precifique seu serviço de forma rápida e ágil, sem necessidade de fazer o seu cadastro.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex-grow">
            <p className="text-sm text-gray-500 mb-6">
              A precificação unitária é utilizada para precificar apenas um serviço específico ou calcular uma margem de lucro para determinada venda total.
            </p>
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1">
                  ...
                  <StarRating
                    ...
                    size={16}
                    moduleType="Precificação Unitária de Serviços"
                  />
                </div>
                <Link href="/precificacao/servicos/unitario">
                  <Button className="flex items-center bg-teal-600 hover:bg-teal-700 text-white">
        ...
        <Card className="flex flex-col">
          <CardHeader>
            ...
            <CardDescription>
              Importe serviços de seu arquivo de dados
            </CardDescription>
          </CardHeader>
          <CardContent className="flex-grow">
            <p className="text-sm text-gray-500 mb-6">
              Importe dados dos seus serviços e ajuste os valores de todos os itens. Com a importação, você consegue ajustar todos eles de forma fácil, sem a necessidade de recadastrar.
            </p>
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1">
                  ...
                  <StarRating
                    ...
                    size={16}
                    moduleType="Importação de Serviços"
                  />
                </div>
                <Link href="/precificacao/servicos/importacao">
                  <Button className="flex items-center bg-teal-600 hover:bg-teal-700 text-white">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Lista de serviços com seus preços e margens
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="mb-4">
                ...
                <TableBody>
                  {servicosPaginados.map(servico => (
                    <TableRow key={servico.id}>
                      ...
                      <TableCell>{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(servico.custo)}</TableCell>
                      <TableCell>{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(servico.precoVenda)}</TableCell>
                      <TableCell>{servico.margem}%</TableCell>
                      <TableCell className="flex space-x-1">
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Link href={`/precificacao/servicos/unitario?servicoId=${servico.id}`}>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-1">
              ...
              <span className="text-base">
                {servicosFiltrados.length > 0 && totalPaginas > 0
