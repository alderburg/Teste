...
export default function SuportePage() {
  ...
  const validateFormFields = (): boolean => {
    ...
    if (camposInvalidos.length > 0) {
      const camposFormatados = camposInvalidos.map(campo => {
        switch(campo) {
          case 'nome': return 'Nome completo';
          case 'email': return formValues.email.trim() === '' ? 'E-mail' : 'E-mail (formato inválido)';
      ...
      toast({
        ...
        description: `Verifique os seguintes campos: ${camposFormatados.join(', ')}`,
        variant: "destructive",
      });
  ...
  const handleTabChange = (value: string) => {
    ...
    // Se mudar para outra aba que não seja "contato", limpar os campos e mensagens de erro
    if (value !== "contato") {
      form.reset();
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Limpar campos ao desmontar o componente
      form.reset();
    };
  }, [form]);
  
  // Efeito para limpar os campos ao sair da página
  useEffect(() => {
    return () => {
      // Limpar formulário e validações ao desmontar o componente
      form.reset();
      setCamposContatoValidados({
        nome: true,
        email: true,
                  ...
                  <CardContent className="pt-6">
                    <Form {...form}>
                      <form onSubmit={form.handleSubmit(onSubmit, (errors) => {
                        ...
                        // Atualizar o estado de validação para exibir as mensagens de erro
                        const formValues = form.getValues();
                        setCamposContatoValidados({
                          nome: !(errors.nome),
                          email: !(errors.email),
                          assunto: !(errors.assunto),
                          categoria: !(errors.categoria),
                          mensagem: !(errors.mensagem)
                        });
                        
                        // Obtém a lista de campos com erro
                        const camposComErro = Object.keys(errors).map(campo => {
                          switch(campo) {
                            case 'nome': return 'Nome completo';
                        ...
                        // Mostra um toast com os campos específicos que possuem erro
                        toast({
                          title: "Erro ao enviar",
                          description: `Verifique os seguintes campos: ${camposComErro.join(', ')}`,
                          variant: "destructive"
                        });
                      })} className="space-y-6">
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem className="space-y-2">
                                ...
                                {!camposContatoValidados.assunto && (
                                  <p className="mt-1 text-red-300 text-xs flex items-center">
                                    <AlertTriangle className="w-3 h-3 mr-1" /> O assunto é obrigatório
                                  </p>
                                )}
                              </FormItem>
                            )}
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem className="space-y-2">
                                ...
                                {!camposContatoValidados.categoria && (
                                  <p className="mt-1 text-red-300 text-xs flex items-center">
                                    <AlertTriangle className="w-3 h-3 mr-1" /> Selecione uma categoria
                                  </p>
                                )}
                              </FormItem>
                            )}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="space-y-2">
                              ...
                              {!camposContatoValidados.mensagem && (
                                <p className="mt-1 text-red-300 text-xs flex items-center">
                                  <AlertTriangle className="w-3 h-3 mr-1" /> A mensagem é obrigatória
                                </p>
                              )}
                            </FormItem>
                          )}
