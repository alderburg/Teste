...
export default function PlanosEUpgradesPage() {
  ...
  const handlePaymentSuccess = () => {
    // Atualização já ocorre via invalidateAssinaturas() no PaymentModal
    // Adicionamos um segundo refetch aqui para garantir 100% que a atualização ocorra
    queryClient.refetchQueries({ queryKey: ['/api/minha-assinatura'] });

    // Mostrar mensagem de sucesso
    toast({
      title: "Pagamento processado com sucesso!",
      description: "Seu plano foi atualizado. Os limites serão atualizados automaticamente.",
      variant: "default"
    });

    // Forçar um reload da página para garantir que todos os limites sejam atualizados corretamente
    // Este é o método mais confiável para garantir a atualização dos valores
    setTimeout(() => {
  ...
  // Componente de loading elegante para a página de planos
      ...
      <div className="text-center space-y-6 p-8">
        ...
        <div className="space-y-3">
          ...
          <p className="text-gray-600 max-w-md mx-auto">
            Estamos buscando os melhores planos para você...
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="bg-white rounded-lg shadow p-6 border-l-4 border-teal-500 h-full flex flex-col">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold text-gray-800">Seu plano atual</h2>
                <Badge variant="outline" className="bg-teal-50 text-teal-700 border-teal-200 py-1">Ativo</Badge>
              </div>
              ...
              <div className="space-y-5">
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-700 mb-3">Detalhes de uso</h3>




                  {/* Utilização de produtos */}
                  <div className="mb-3 p-3 bg-purple-50 rounded-lg border border-purple-100 shadow-sm">
                    <div className="flex items-center mb-2">
                      <Users className="h-4 w-4 text-purple-600 mr-2" />
                      <span className="text-sm text-purple-700 font-medium">Produtos</span>
                    </div>

                    <div className="flex justify-between mb-1">
                      {/*<span className="text-xs text-purple-600 font-medium">Cadastrados</span>*/}
                      <span className="text-xs text-purple-800 font-semibold">
                        {assinaturaData?.estatisticas?.produtosCadastrados || 1} de {' '}
                        {assinaturaData?.plano?.limitesCadastro?.produtos === 'Ilimitado' 
                          ...
                          : (typeof assinaturaData?.plano?.limitesCadastro?.produtos === 'number'
                              ...
                              : assinaturaData?.plano?.nome === 'ESSENCIAL' ? 50
                              : assinaturaData?.plano?.nome === 'PROFISSIONAL' ? 250
                              : assinaturaData?.plano?.nome === 'EMPRESARIAL' ? 500
                              : assinaturaData?.plano?.nome === 'PREMIUM' ? 'Ilimitado'
                              : 5)}
                      </span>
                    </div>
                    <div className="relative w-full h-2.5 bg-purple-100 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ 
                          width: assinaturaData?.plano?.limitesCadastro?.produtos === 'Ilimitado' || 
                                 assinaturaData?.plano?.nome === 'PREMIUM'
                            ? '0%' // Para planos ilimitados, mostramos um valor fixo 
                            : `${Math.min(100, ((assinaturaData?.estatisticas?.produtosCadastrados || 1) * 100) / 
                                (typeof assinaturaData?.plano?.limitesCadastro?.produtos === 'number' 
                                  ? assinaturaData?.plano?.limitesCadastro?.produtos
                                  : assinaturaData?.plano?.nome === 'ESSENCIAL' ? 50
                                  : assinaturaData?.plano?.nome === 'PROFISSIONAL' ? 250
                  ...
                  <div className="mb-3 p-3 bg-purple-50 rounded-lg border border-purple-100 shadow-sm">
                    <div className="flex items-center mb-2">
                      <Users className="h-4 w-4 text-purple-600 mr-2" />
                      <span className="text-sm text-purple-700 font-medium">Usuários</span>
                    </div>
                      ...
                      <span className="text-xs text-purple-800 font-semibold">
                        ...
                        {assinaturaData?.plano?.limitesCadastro?.usuarios === 'Ilimitado' 
                          ...
                          : (typeof assinaturaData?.plano?.limitesCadastro?.usuarios === 'number'
                              ...
                              : assinaturaData?.plano?.nome === 'ESSENCIAL' ? 1
                              : assinaturaData?.plano?.nome === 'PROFISSIONAL' ? 3
                              : assinaturaData?.plano?.nome === 'EMPRESARIAL' ? 5
                              : assinaturaData?.plano?.nome === 'PREMIUM' ? 'Ilimitado'
                              : 5)}
                      </span>
                    </div>
                    <div className="relative w-full h-2.5 bg-purple-100 rounded-full overflow-hidden">
                      <div 
                        className="absolute top-0 left-0 h-full bg-purple-500 rounded-full" 
                        style={{ 
                          width: assinaturaData?.plano?.limitesCadastro?.usuarios === 'Ilimitado' || 
                                 assinaturaData?.plano?.nome === 'PREMIUM'
                            ? '0%' // Para planos ilimitados, mostramos um valor fixo 
                  ...
                  {temAssinatura && assinaturaData?.assinatura && (
                    <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-100 shadow-sm flex items-center">
                      <Calendar className="h-5 w-5 text-blue-600 mr-2" />
                      <span className="text-sm font-semibold text-blue-800">{calcularDiasRestantes(assinaturaData.assinatura)}</span>
                    </div>
                  )}
                </div>
              </div>
