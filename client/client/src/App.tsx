...
function Router() {
  ...
  // Efeito para carregar dados iniciais e configurar a aplicação
  useEffect(() => {
    // ESTRATÉGIA DEFINITIVA PARA LOGOUT: verificar várias condições que podem indicar
    // um processo de logout e remover o splash-screen completamente
    const isDuringLogout = 
      sessionStorage.getItem('isLogoutRedirect') === 'true' || 
      sessionStorage.getItem('bypassAllSplashScreens') === 'true' || 
      new URLSearchParams(window.location.search).get('fromdirectlogout') === 'true';
    
    // NOVA FLAG GLOBAL: verificar IMEDIATAMENTE se estamos no processo de logout
    // e definir uma variável global para outros componentes consultarem
    if (isDuringLogout) {
      // Definir flag global 
      (window as any).DURING_LOGOUT_PROCESS = true;
      
      // Remover SEMPRE o splash screen neste caso
      console.log("Processo de logout detectado - removendo splash screen completamente");
      
      // Remover imediatamente qualquer splash screen existente
      const splash = document.getElementById('splash-screen');
      if (splash && splash.parentNode) {
        splash.parentNode.removeChild(splash);
      }
      
      return; // Interromper processamento neste ponto
    }
    
    // Verificar se é necessário FORÇAR a exibição do splash screen (funcionalidade antiga)
    // mas apenas se NÃO estamos em processo de logout
    const forceShowSplash = !isDuringLogout && sessionStorage.getItem('forceShowSplash') === 'true';
    
    // Se forçar exibição do splash, remover esse flag para não afetar navegações futuras
    if (forceShowSplash) {
      console.log("Flag forceShowSplash detectado - mostrando splash screen");
      sessionStorage.removeItem('forceShowSplash');
      
      // Não fazer mais nada - deixar o splash screen ser exibido normalmente
      // e depois substituído pela página de login via comportamento normal da aplicação
      return;
    }
    
    // Verificar TODOS os flags de controle de splash screen (caso não tenha flag forceShowSplash)
    const shouldSkipSplash = 
      ...
      (new URLSearchParams(window.location.search).get('noSplash') === 'true');
    
    // Verificar variável global que pode ter sido definida em outro contexto
    const globalSplashDisabled = !!(window as any).DISABLE_ALL_SPLASH_SCREENS;
    
    // Verificar parâmetros da URL para controle adicional
    const urlParams = new URLSearchParams(window.location.search);
    const fromLogout = urlParams.get('logout') === 'true';
    
    // Remover splash screen nos casos normais
    const splash = document.getElementById('splash-screen');
    if (splash) {
      if (shouldSkipSplash || globalSplashDisabled || fromLogout) {
        console.log("Splash screen desabilitado por flag de controle ou parâmetro URL.");
        
        // Remover completamente do DOM para garantia total
        splash.parentNode?.removeChild(splash);
      } else {
        // Comportamento normal para outros casos (mais suave)
        splash.style.opacity = '0';
        splash.style.display = 'none';
      }
    }
    
    // Definir variável global para controlar splash em outros contextos
    if (shouldSkipSplash || fromLogout) {
      (window as any).DISABLE_ALL_SPLASH_SCREENS = true;
    }
  ...
  // PROTEÇÃO CRÍTICA: Todas as rotas agora exigem autenticação, exceto rotas exatas de auth e landing page
  if (isLandingPage) {
    return (
