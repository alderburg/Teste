...
export default function EnderecosTab() {
  ...
  const { data: enderecosData, isLoading: isLoadingEnderecos, error: enderecosError, refetch: refetchEnderecos } = useQuery({
    ...
    <Card className="shadow-sm">
      ...
      <CardContent>
        <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center mb-4 gap-2">
          <div>
            <h3 className="text-base font-medium">Endereços cadastrados</h3>
            <p className="text-xs text-gray-500 mt-1">Adicione, edite ou remova endereços</p>
          </div>
          {!showAddEndereco && (
            <Button 
              onClick={() => {
