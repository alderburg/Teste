...
// Inicializar a API do Stripe globalmente para uso em toda a aplicação
const stripe = process.env.STRIPE_SECRET_KEY 
  ? new Stripe(process.env.STRIPE_SECRET_KEY, { apiVersion: '2023-10-16' as any }) 
  : null;

// Mensagem inicial de status do Stripe (será atualizada após validação assíncrona)
console.log('Status inicial do Stripe:', stripe ? 'Inicializado - Validando conexão...' : 'FALHA - Não foi possível inicializar');
...
// Endpoint para verificar a senha atual do usuário usando header de simulação
app.post("/api/auth/verify-password", async (req, res) => {
  try {
    const { password, userId } = req.body;
    
    // Verificar se temos password e userId
    if (!password || !userId) {
    ...
    if (!user) {
      console.error(`Verificação de senha: usuário ${userId} não encontrado`);
      return res.status(404).json(false);
    }

    // Verificação rigorosa e segura
...
// ENDPOINT DIRETO para verificação de senha completa
app.post("/api/password/verify", async (req, res) => {
  try {
    ...
    // Para facilitar testes, permitir senha de teste
...
app.post("/api/password/verify-partial", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        await storage.createAssinatura({
          ...
          tipoCobranca,
          valorPago: valorPlano.toString(), // Converter para string conforme esperado pelo schema
          status: 'pendente', // Será atualizado quando o pagamento for confirmado
          dataFim: dataFim
        });
  ...
  async function handleInvoicePaymentFailed(invoice: any) {
    try {
      // Obter a assinatura relacionada à fatura
      const subscription = await stripe?.subscriptions.retrieve(invoice.subscription);
      ...
      console.log(`Assinatura ${subscription.id} atualizada para status: inadimplente`);
    } catch (error) {
  ...
  async function handleSubscriptionCanceled(subscription: any) {
    try {
      ...
      let dataFim = new Date(dataFimUTC.getTime() - (3 * 60 * 60 * 1000)); // Horário brasileiro (UTC-3)
      
      // Se a assinatura tiver uma data de fim de período atual, usar essa data
      if (subscription.current_period_end) {
        const dataFimPeriodoUTC = new Date(subscription.current_period_end * 1000);
        // 🇧🇷 Converter para horário brasileiro (UTC-3)
        const dataFimPeriodo = new Date(dataFimPeriodoUTC.getTime() - (3 * 60 * 60 * 1000));
        
        // Comparar com a data atual - se o período já terminou, usar a data do período
        // Se ainda não terminou e o cancelamento é imediato, usar a data atual
        if (dataFimPeriodo > dataFim && !subscription.cancel_at_period_end) {
          // Cancelamento imediato
          console.log(`Cancelamento imediato da assinatura ${subscription.id}`);
        } else if (subscription.cancel_at_period_end) {
          // Cancelamento no fim do período
          dataFim = dataFimPeriodo;
          console.log(`Assinatura ${subscription.id} será cancelada no fim do período: ${dataFimPeriodo.toISOString()}`);
        }
      }
